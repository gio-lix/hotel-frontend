...
const Hotel = () => {
    ...
    const { data }: any = useFetch(`/hotels/find/${id}`);
    ...
    return (
        <section className={clsx('container', "hotel")}>
            <article className="hotel-title-box">
                <h1>{data.name}</h1>
                <div  className="articleBox_location" >
                    <span>
                        <ImLocation />
                    </span>
            ...
            <div className={clsx('flex', "hotel-box")}>
                {activeImage ? (
                    <div className="hotel-active-image">
                        <img src={images[activeImageIndex].image} alt="images"/>
                        <span className="hotel-cansel" onClick={() => setActiveImage(false)}>
                            <AiOutlineClose />
                        </span>
                        <div className="hotel-image-button">
                            <button onClick={() => onHandleChangeIndex("minus")}>
                                <AiOutlineLeft />
                            </button>
                            <button onClick={() => onHandleChangeIndex("plus")}>
                                <AiOutlineLeft className="rotate"/>
                            </button>
                        </div>
                    </div>
                ) : null}
                <div className="hotel-grid" style={{flexGrow: 1}}>
                ...
                <aside className="hotel-data-length-box">
                    <h3>
